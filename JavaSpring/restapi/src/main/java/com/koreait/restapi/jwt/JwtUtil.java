...
public class JwtUtil {
    ...
    public String generateToken(String username, int userId) {
        return Jwts.builder()
                .setSubject(username)                                                 // 토큰 주체(사용자 이름)를 설정
                .claim("id", userId)                                                  // 사용자 ID를 추가 정보로 저장
    ...
    public Integer getUserIdFromToken(String token) {
        return parseClaims(token).get("id", Integer.class);                           // JWT Claims에서 id 값 가져오기
    }
...
1. 클라이언트가 서버에 로그인 요청 시:
   - 서버는 사용자 식별 정보(예: 사용자명, ID)를 포함해 JWT 토큰을 생성한다.
   - 이 때 서버는 비밀키(SECRET)를 이용하여 JWT의 서명을 만든다.

2. 클라이언트는 서버에서 받은 JWT를 로컬에 저장하고, 요청 시마다 HTTP 헤더("Authorization: Bearer [JWT]")에 담아 보낸다.

3. 서버가 요청을 받으면:
   - JWT 토큰을 헤더에서 추출하여 비밀키(SECRET)를 통해 서명 유효성을 검증한다.
   - 유효한 토큰이면 Claims에서 사용자 정보를 추출해 사용자를 인증 처리한다.
   - 유효하지 않거나 만료된 토큰이면 인증 실패로 처리한다.

즉, JWT는 인증된 사용자임을 확인하고, 서버가 요청을 처리할 때 신뢰할 수 있는 사용자 정보를 담아 전달하는 인증 메커니즘
*/
