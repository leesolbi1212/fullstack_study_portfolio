...
//DB와 직접 연결하여 회원정보 CRUD 작업을 수행하는 클래스
public class MemberDAO {
    ...
    public MemberDTO login(String userid, String userpw) {
        ...
        // 회원정보 수정 메서드 (result에서 넘겨준 dto 객체 안의 정보를 DB에 업데이트)
        ...
        """;
            ...
            try(Connection conn = DBUtil.getConnection();
                ...
                pstmt.setString(12, member.getUserid()); // userid는 WHERE절에 들어가야 합니다.
    ...
    // getMemberById : 회원정보를 DB에서 조회하는 메서드
    public MemberDTO getMemberById(String userid) {
//        String sql = """
//        SELECT userid, userpw, name, hp, email, gender, ssn1, ssn2, zipcode, address1, address2, address3
//        FROM member
//        WHERE userid = ?
//        """;
...
//            try (ResultSet rs = pstmt.executeQuery()) { // 결과를 받기 위한 ResultSet
                if (rs.next()) {  // 데이터가 있으면 DTO에 담아서 반환
                    MemberDTO member = new MemberDTO();
                    member.setUserid(rs.getString("userid"));
    ...
    // DB에서 회원정보를 삭제하는 메서드 추가
    public boolean deleteMember(String userid) {
        String sql = "delete from member where userid = ?";

        // try-with-resources로 리소스를 자동 관리
        try(Connection conn = DBUtil.getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql)) {

            pstmt.setString(1, userid); // SQL에 userid 설정

            // executeUpdate로 삭제 처리, 성공 시 1 반환
            return pstmt.executeUpdate() == 1;
